...
const searching = async () => {
    if (search.value == '') {
        ...
        const res = await axios.post('/api/admin/inventory/lingayen/search', { search: search.value,catid: cat_id.value });
...
const add = async () => {
   ...
   try {
    if (updateButton.value) {
        const res = await axios.post('/api/admin/inventory/lingayen/update/' + ids.value, data);
        Swal.fire({
            position: 'center',
            icon: 'success',
        ...
        const res = await axios.post('/api/admin/inventory/lingayen/add', data)
...
const fetch = async (pageNum) => {
    ...
    const res = await axios.get('/api/admin/inventory/lingayen/all?page=' + pageNum+'&catid='+cat_id.value);
    allData.value = res.data.data;
    pageCount.value = res.data.last_page;
    page.value = res.data.current_page;
    perPage.value = res.data.per_page;
...
const fetchCat = async (pageNum) =>{
    ...
    const res = await axios.get('/api/admin/inventory/lingayen/all/cat?page=' + pageNum);
...
const destroy = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            axios.delete('/api/admin/inventory/lingayen/delete/' + id).then(res => {
                fetchCat();
                fetch();
                Swal.fire(
                    'Deleted!',
