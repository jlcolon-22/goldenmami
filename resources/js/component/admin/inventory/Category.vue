        ...
        <div class="grid grid-cols-2 gap-5 p-10">
            ...
            <form @submit.prevent="add" class="bg-white shadow h-fit">
                <div class="bg-green-600 flex justify-between items-center px-2">
                    <h1 v-if="!updateButton" class="text-lg font-bold font-inter p-2 text-gray-200 ">ADD ITEM CATEGORY</h1>
                    <h1 v-else class="text-lg font-bold font-inter p-2 text-gray-200 ">EMPLOYEE ITEM CATEGORY</h1>
...
const add = async () =>{
    try {
        ...
        {
            const res = await axios.post('/api/admin/inventory/category/update/'+ids.value,data);
            fetch();
            Swal.fire({
                position: 'center',
                icon: 'success',
            ...
            const res = await axios.post('/api/admin/inventory/category',data);
            fetch();
            Swal.fire({
                position: 'center',
                icon: 'success',
...
const fetch = async (pageNum) =>{
    try {
        const res = await axios.get('/api/admin/inventory/categories?page='+pageNum);
...
const destroy = async (id) =>{
    try {
        ...
        if (result.isConfirmed) {
            axios.delete('/api/admin/inventory/category/delete/'+id).then(res =>{
                fetch()
                Swal.fire(
            'Deleted!',
