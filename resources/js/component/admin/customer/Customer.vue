            ...
            <div class="bg-[#fff] shadow rounded min-w-[70%]" style="max-width: 70%;">
                ...
                <div class="p-5 flex justify-between items-start">
                    ...
                    <div class="w-full  p-5">
                        <h1 class="font-inter text-lg font-bold">Nickname: <span class="font-inter font-normal text-base">{{ data.nickname }}</span></h1>
                        ...
                        <h1 class="font-inter text-lg font-bold ">Email Validate: <span class="font-inter font-normal text-base bg-[#ebb700] py-1 px-2 rounded">{{ data.email_validate }}</span></h1>
       ...
       <div v-show="addModal"
            ...
            <form @submit.prevent="add" class="bg-white shadow rounded min-w-[70%] h-fit " style="max-width: 70%"
                ...
                <div class="bg-[#ebb700] flex justify-between items-center px-2">
                    <h1 v-if="!updateButton" class="text-lg font-bold font-inter p-2 text-gray-200 ">ADD CUSTOMER</h1>
                    <h1 v-else class="text-lg font-bold font-inter p-2 text-gray-200 ">UPDATE CUSTOMER</h1>
                    ...
                    <div class=" grid grid-cols-1 gap-3 w-full">
                            ...
                            <div>
                                ...
                                <input type="text" v-model="data.nickname"
                                    ...
                                    placeholder="ex: Leni">
                                <small v-if="errors.nickname" class="text-red-600 font-inter">{{ errors.nickname[0]
                                }}</small>
                            </div>
                        ...
                        <div>
                            <label>Email Validate</label>
                                <select class="py-2 px-4 bg-gray-200 rounded w-full inline-block font-inter" v-model="data.email_validate">
                                    <option value="">Choose</option>
                                    <option value="1">Validate</option>
                                    <option value="0">Not Validate</option>
                                </select>
                            <small v-if="errors.email_validate" class="text-red-600 font-inter">{{ errors.email_validate[0] }}</small>
                        </div>
                        </div>
        ...
        <div class="relative overflow-x-auto">
            <table class="w-full text-sm text-left text-gray-500 z-0">
                ...
                <tbody>
                    <tr class="bg-white border-b " v-for="customer in allData.data" :key="customer.id">
...
const searching = async () =>{
        if(search.value == '')
        {
            ...
            const res = await axios.post('/api/admin/customer/search',{search: search.value});
...
const add = async () => {
    ...
    if(updateButton.value)
    {
        ...
        const res = await axios.post('/api/admin/customer/update/'+ids.value,formData);
        Swal.fire({
            position: 'center',
            icon: 'success',
...
const view = (id) =>{
    ...
    {
        data.photo = 'https://eu.ui-avatars.com/api/?background=random&name='+datas[0].nickname
    }else{
