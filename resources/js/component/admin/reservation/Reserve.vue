<template>
    ...
    <form @submit.prevent="add" class="bg-white shadow rounded min-w-[70%] h-fit " enctype="multipart/form-data">
        ...
        <div v-show="viewModal"
            ...
            <div
                ...
                <div>
                    ...
                    <h1 class="font-inter"><span class="font-bold text-gray-400 font-inter text-sm">Nickname -</span> {{
                        !!viewData[0] ? viewData[0][0].customer_info.nickname : '' }}</h1>
                    <h1 class="font-inter"><span class="font-bold text-gray-400 font-inter text-sm">Email -</span> {{
                        !!viewData[0] ? viewData[0][0].customer_info.email : '' }}</h1>
                    <h1 class="font-inter"><span class="font-bold text-gray-400 font-inter text-sm">Number -</span> {{
                        !!viewData[0] ? viewData[0][0].customer_info.number : '' }}</h1>
                    ...
                    <h1 class="font-inter"><span class="font-bold text-gray-400 font-inter text-sm"># -</span> {{
                        !!viewData[0]
                        ?
                        viewData[0][0].id : '' }}</h1>
                    <h1 class="font-inter"><span class="font-bold text-gray-400 font-inter text-sm">DATE -</span> {{
                        !!viewData[0] ?
                        viewData[0][0].date : '' }}</h1>
                    <h1 class="font-inter"><span class="font-bold text-gray-400 font-inter text-sm">GCASH CHECKOUT ID
                            -</span>
                        {{ !!viewData[0] ?
                            viewData[0][0].cs_id : '' }}</h1>
                    <h1 class="font-inter"><span class="font-bold text-gray-400 font-inter text-sm">TIME -</span> {{
                        !!viewData[0] ?
                        tConv24(viewData[0][0].time) : '' }}</h1>
                    <h1 class="font-inter"><span class="font-bold text-gray-400 font-inter text-sm">NUMBER OF GUEST -</span>
                        {{
                            !!viewData[0] ? viewData[0][0].guest : '' }}</h1>
                    <h1 class="font-inter flex items-center gap-1"><span
                            class="font-bold flex text-gray-400 font-inter text-sm">BRANCH -</span> <span
                            v-if="!!viewData[0]">
                            <h2 class="bg-yellow-600 text-gray-100 px-5 py-1 rounded w-fit"
                    ...
                    <h1 class="font-inter"><span class="font-bold text-gray-400 font-inter text-sm">STATUS -</span>
                        <span v-if="!!viewData[0]">
                            <span v-if="viewData[0][0].status == 0"
                                class="text-yellow-400 p-2 rounded font-bold text-lg font-inter">processing</span>
                            <span v-if="viewData[0][0].status == 1"
                                class="text-green-400 p-2 rounded font-bold text-lg font-inter">Aproved</span>
                            <span v-if="viewData[0][0].status == 2"
                                class="text-red-400 p-2 rounded font-bold text-lg font-inter">Reject</span>
                        </span>
                    </h1>
                    <h1 class="font-inter"><span class="font-bold text-gray-400 font-inter text-sm">PAYMENT STATUS -</span>
                        {{
                            !!viewData[0] ? viewData[0][0].payment_status : '' }}</h1>
                    <h1 class="font-inter"><span class="font-bold text-gray-400 font-inter text-sm">DOWNPAYMENT -</span>₱{{
                        !!viewData[0] ? viewData[0][0].payment : '' }}</h1>
                    <hr class="bg-gray-700 my-3 h-[2px]">
                </div>
                <div>
                    ...
                    <div class="flex justify-between">
                        ...
                        <button @click="viewModal = false"><img src="/svg/exit.svg" class="hover:opacity-100 opacity-60"
                                alt=""></button>
                    </div>
                    <div class="">
                        ...
                        <div class="grid grid-cols-1 gap-y-2 overflow-y-auto max-h-[180px] ">
                            <div class="flex justify-between  items-center" v-for="reservationData in viewData"
                                :key="reservationData.id">
                                <div class="w-full">
                                    <img :src="`/storage/menu/${reservationData[1].photo}`"
                                        class="w-[60px] h-[50px] rounded object-cover" alt="">
                                </div>
        ...
        <div  v-show="loading"
            ...
            <div class="border-t-[6px] flex items-center border-t-blue-600 bg-white w-[200px] h-[80px] px-4 py-2">
        ...
        <div class="py-2 flex justify-end  ">
                ...
                <input type="date" @change="search()"  class="py-2 px-4 bg-gray-300 rounded w-[200px]" placeholder="search">

        </div>
        <div class=" w-full">

            <table class="text-sm text-left text-gray-500 z-0 overflow-hidden w-full">
                ...
                <tbody>
                    <tr class="bg-white border-b " v-for="item in allData.data" :key="item.id">
                        ...
                        <th scope="row"
                            ...
                            <span v-if="item.status == 0"
                                class="bg-yellow-400 p-2 rounded text-white text-xs font-inter">processing</span>
                            <span v-if="item.status == 1"
                                class="bg-green-400 p-2 rounded text-white text-xs font-inter">Aproved</span>
                            <span v-if="item.status == 2"
                                class="bg-red-400 p-2 rounded text-white text-xs font-inter">Reject</span>


                        </th>
                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap   font-inter">
                            <span v-if="item.payment_status == 'paid'"
                                class="p-2 rounded opacity-70 font-bold text-sm font-inter">PAID</span>
                            <span v-if="item.payment_status == 'failed'"
                                class="p-2 rounded text-red-500 font-bold text-sm font-inter">FAILED</span>
                        </th>
                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap   font-inter">
                            <span v-if="item.branch == 1"
                                class="bg-yellow-400 p-2 rounded text-white text-xs font-inter">CALASIAO</span>
                            <span v-if="item.branch == 2"
                                class="bg-blue-400 p-2 rounded text-white text-xs font-inter">DAGUPAN</span>
                            <span v-if="item.branch == 3"
                        ...
                        <td class="px-6 py-4  font-inter">
                            <button @click="view(item.id)"
                                class="  p2-4 py-2 font-bold text-sm font-inter hover:opacity-70 rounded inline-block mr-2"><img
                                    src="/svg/view.svg" alt=""></button>
                                    <button @click="destroy(item.id)"
                                class="  p2-4 py-2 font-bold text-sm font-inter hover:opacity-70 rounded inline-block mr-2"><img
                                    src="/svg/delete.svg" alt=""></button>

                        </td>
                        <td class="px-6 py-4  font-inter">

                                <button title="aprroved" @click="approvedReject(item.id, 1)"
                                    class=" inline px-2 py-2 font-bold text-sm font-inter hover:opacity-70 rounded"><img
                                        src="/svg/check.svg" alt=""></button>

                                <button title="reject" @click="approvedReject(item.id, 2)"
                                    class="  px-2 py-2 font-bold text-sm font-inter hover:opacity-70 rounded inline-block mr-2"><img
                                        src="/svg/exit.btn.svg" alt=""></button>
            ...
            <div class="py-3">
                <paginate :active-class="'text-blue-600 border border-gray-300 bg-blue-200 font-bold'"
                    :page-count="pageCount" :page-range="perPage" :margin-pages="2" :click-handler="fetch"
                    :prev-text="'Prev'" :next-text="'Next'" disabled-class="opacity-50 disabled outline-none"
...
const search = async ()=>{
    if(event.target.value == '')
    {
        ...
        try {
        const res = await axios.get('/api/admin/reservation/search?s='+event.target.value);
...
const fetch = async (pageNum) => {
    try {
        const res = await axios.get('/api/admin/reservation/list?page=' + pageNum);
        allData.value = res.data;
...
const approvedReject = async (id, status) => {
    loading.value = true;
    const res = await axios.post(`/api/admin/reservation/approved/${id}`, { data: status });
    await fetch()
     loading.value = false;
    Swal.fire({
            position: 'center',
...
const d = new Date(dt.split('-')[0], dt.split('-')[1] - 1,dt.split('-')[2])
